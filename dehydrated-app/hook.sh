...
check_status() {
  ...
  if [[ "$status" != 'true' ]]; then
    if [[ "$status" == 'false' ]]; then
      echo "${response[@]}" | jq -r '.errors | .[] | .message' | while read line; do
        echo '!!! ERROR: '"$line"
      done
    else
      echo "${response[@]}"
...
deploy_challenge() {
  ...
  while (($# > 0)); do
    ...
    # load_config "$DOMAIN"
    configFile="${DIR}/domain-config.sh"

    . "${configFile}"

    echo "   - Setting up token for ${DOMAIN} / $zone_id"
    local DATA='{"type":"TXT","name":"'${prefix}${DOMAIN}'","content":"'${TOKEN_VALUE}'","ttl":120,"priority":10,"proxied":false}'
    CURL_CMD="${DIR}/cloudflare-curl.sh -m POST -p dns_records -z '${zone_id}' -t '${api_token}' '${DATA}'"
    result=$(echo $CURL_CMD | bash)
...
clean_challenge() {
  ...
  while (($# > 0)); do
    ...
    CURL_CMD="${DIR}/cloudflare-curl.sh -z '${zone_id}' -t '${api_token}' -p 'dns_records?match=all&per_page=100&type=TXT&name=${prefix}${DOMAIN}'"
    response=$(echo $CURL_CMD | bash)
    ...
    for id in ${record_ids}; do
      ...
      CURL_CMD="${DIR}/cloudflare-curl.sh -m DELETE -z '${zone_id}' -t '${api_token}' -p dns_records/${id}"
      response=$(echo $CURL_CMD | bash)
      echo $response | check_status
    done
